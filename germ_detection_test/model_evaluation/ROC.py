...
绘制某一类别的ROC曲线
'''
def roc_one_category(specific_class,pred_of_test):
    # 二分类标注
    y_test = (pred_of_test['标注类别名称'] == specific_class)
    # 二分类置信度
    y_score = pred_of_test['positive-预测置信度']
...
绘制所有类别的ROC曲线
'''
def roc_all_categories(pred_of_test):
    ...
    for each_class in classes:
        y_test = list((pred_of_test['标注类别名称'] == each_class))
        y_score = list(pred_of_test['{}-预测置信度'.format(each_class)])
        fpr, tpr, threshold = roc_curve(y_test, y_score)
    ...
    df_report = pd.read_csv('../resources/form/各类别准确率评估指标.csv')
    ...
    df_report.to_csv('../resources/form/各类别准确率评估指标.csv', index=False)
...
if __name__ == '__main__':
    ...
    roc_one_category('positive',df)

    roc_all_categories(df)