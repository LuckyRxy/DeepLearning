...
idx_to_labels = np.load('../../resources/output/idx_to_labels.npy', allow_pickle=True).item()
# 获得类别名称
classes = list(idx_to_labels.values())
print(classes)

# 载入训练好的模型
model = torch.load('../../resources/checkpoint/best-0.892.pth')
...
df.to_csv('../../resources/output/测试集预测结果.csv', index=False)
...
df = pd.read_csv('../../resources/output/测试集预测结果.csv')
...
df_report.to_csv('../../resources/output/各类别准确率评估指标.csv', index_label='类别')
